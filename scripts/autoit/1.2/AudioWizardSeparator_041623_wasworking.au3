...
Func _TabHandler()
    ...
    If $sDefaultModel <> "" Then
        If _IsModelCompatibleWithTab($sDefaultModel, $iTabIndex) Then
            ...
            Local $sSelectedModel = GUICtrlRead($hModelCombo) ; Declare once
            If $sSelectedModel = $sDefaultModel Then
                _Log("Default model " & $sDefaultModel & " set successfully")
                Local $aDetails = _GetModelDetails($sDefaultModel) ; Declare here
                If Not @error Then
        ...
        Else
            ...
            If $sFirstModel <> "" And $sFirstModel <> "No models available" Then
                ...
                Local $sSelectedModel = GUICtrlRead($hModelCombo) ; Reuse $sSelectedModel
                If $sSelectedModel = $sFirstModel Then
                    Local $aDetails = _GetModelDetails($sFirstModel) ; Declare here
                    If Not @error Then
