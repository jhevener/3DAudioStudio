...
; Declare GUI controls to suppress warnings
Global $hSegmentSizeLabel = 0, $hSegmentSizeInput = 0, $hOverlapLabel = 0, $hOverlapInput = 0
Global $hDenoiseCheckbox = 0, $hBatchSizeLabel = 0, $hBatchSizeInput = 0
; Declare $sSelectedModel to prevent redeclaration warning
...
Func _InitializeModels()
    ...
    If Not FileExists($sDbFile) Then
        ...
        Local $sName, $sApp, $sFocus, $iStems, $sPath, $sDescription, $sComments, $sCommandLine
        If @error Or Not IsArray($aSections) Then
            _Log("Failed to read Models.ini or file is missing: " & $sModelsIni, True)
            ; Insert default Demucs model (htdemucs)
            Local $iModelID = 4
            $sQuery = 'INSERT INTO Models (ModelID, Name, Path, Description, Comments, CommandLine) VALUES (' & $iModelID & ', ''htdemucs'', ''N/A'', ''Demucs model for separating audio into vocals, drums, bass, and other.'', ''Good for 4-stem separation but may muffle or phase audio in some genres; test with VR models for comparison.'', ''cmd /c "cd @ScriptDir@\\installs\\Demucs\\demucs_env\\Scripts && activate.bat && python.exe -m demucs -o \"@OutputDir@\" \"@SongPath@\" && deactivate"'');'
            _SQLite_Exec($hDb, $sQuery)
            If @error Then
            ...
            $sQuery = "INSERT INTO ModelApps (ModelID, App) VALUES (" & $iModelID & ", 'Demucs');"
            _SQLite_Exec($hDb, $sQuery)
            If @error Then
            ...
            $sQuery = "INSERT INTO ModelFocuses (ModelID, Focus, Stems) VALUES (" & $iModelID & ", 'Vocals, Drums, Bass, Other', 4);"
            _SQLite_Exec($hDb, $sQuery)
            If @error Then
            ...
            ; Insert default Spleeter model (2stems)
            $iModelID = 2
            $sName = "2stems"
            $sApp = "Spleeter"
            ...
            ; Insert default UVR5 model (UVR-MDX-NET-Inst_Main)
            $iModelID = 3
            $sName = "UVR-MDX-NET-Inst_Main"
            $sApp = "UVR5"
            $sFocus = "Instrumental"
            $iStems = 1
            $sPath = "@ScriptDir@\installs\UVR\models\UVR-MDX-NET-Inst_Main.onnx"
            $sDescription = "MDX-Net model for isolating main instrumental tracks."
            $sComments = "Effective for instrumental extraction; use in ensemble mode with vocal models for balanced results."
            $sCommandLine = ""
            $sQuery = "INSERT INTO Models (ModelID, Name, Path, Description, Comments, CommandLine) VALUES (" & $iModelID & ", '" & $sName & "', '" & $sPath & "', '" & $sDescription & "', '" & $sComments & "', '" & $sCommandLine & "');"
            _SQLite_Exec($hDb, $sQuery)
            If @error Then
                _Log("Failed to insert UVR5 model into Models: " & _SQLite_ErrMsg(), True)
                _SQLite_Close($hDb)
                _SQLite_Shutdown()
                Return SetError(12, 0, False)
            EndIf
            _Log("Inserted UVR5 ModelID " & $iModelID & " into Models")
            $sQuery = "INSERT INTO ModelApps (ModelID, App) VALUES (" & $iModelID & ", '" & $sApp & "');"
            _SQLite_Exec($hDb, $sQuery)
            If @error Then
                _Log("Failed to insert into ModelApps for UVR5 model: " & _SQLite_ErrMsg(), True)
                _SQLite_Close($hDb)
                _SQLite_Shutdown()
                Return SetError(13, 0, False)
            EndIf
            _Log("Inserted UVR5 ModelID " & $iModelID & " into ModelApps")
            $sQuery = "INSERT INTO ModelFocuses (ModelID, Focus, Stems) VALUES (" & $iModelID & ", '" & $sFocus & "', " & $iStems & ");"
            _SQLite_Exec($hDb, $sQuery)
            If @error Then
                _Log("Failed to insert into ModelFocuses for UVR5 model: " & _SQLite_ErrMsg(), True)
                _SQLite_Close($hDb)
                _SQLite_Shutdown()
                Return SetError(14, 0, False)
            EndIf
            _Log("Inserted UVR5 ModelID " & $iModelID & " into ModelFocuses")

            ; Insert default UVR5 model (1_HP-UVR)
            $iModelID = 5
            $sName = "1_HP-UVR"
            $sApp = "UVR5"
            $sFocus = "Vocals"
            $iStems = 2
            $sPath = "@ScriptDir@\installs\UVR\models\1_HP-UVR.pth"
            $sDescription = "High-performance VR model for vocal separation, version 1."
            $sComments = "Great for intense genres like screamo; use Window Size 1024, Aggression Level 1 to reduce phasing."
            $sCommandLine = ""
            $sQuery = "INSERT INTO Models (ModelID, Name, Path, Description, Comments, CommandLine) VALUES (" & $iModelID & ", '" & $sName & "', '" & $sPath & "', '" & $sDescription & "', '" & $sComments & "', '" & $sCommandLine & "');"
            _SQLite_Exec($hDb, $sQuery)
            If @error Then
            ...
            ; Insert default UVR5 model (Kim_Vocal_1)
            $iModelID = 6
            $sName = "Kim_Vocal_1"
            $sApp = "UVR5"
            $sFocus = "Vocals"
            $iStems = 1
            $sPath = "@ScriptDir@\installs\UVR\models\Kim_Vocal_1.onnx"
            $sDescription = "MDX-Net model for isolating vocals, version 1."
            $sComments = "Good starting point for vocal extraction; try chaining with VR de-reverb and de-echo models."
            $sCommandLine = ""
            $sQuery = "INSERT INTO Models (ModelID, Name, Path, Description, Comments, CommandLine) VALUES (" & $iModelID & ", '" & $sName & "', '" & $sPath & "', '" & $sDescription & "', '" & $sComments & "', '" & $sCommandLine & "');"
            _SQLite_Exec($hDb, $sQuery)
            If @error Then
        ...
        Else
            ...
            For $i = 1 To $aSections[0]
                ...
                Local $sModelName = IniRead($sModelsIni, $sSection, "Name", $sSection)
                ...
                If $sModelApp = "" Then
                    _Log("Skipping invalid model entry (missing App): " & $sSection, True)
                    ContinueLoop
                EndIf

                $sQuery = "INSERT INTO Models (Name, Path, Description, Comments, CommandLine) VALUES ('" & $sModelName & "','" & $sModelPath & "','" & $sModelDesc & "','" & $sModelComments & "','" & $sModelCmd & "')"
                _SQLite_Exec($hDb, $sQuery)
                If @error Then
                ...
                $sQuery = "INSERT INTO ModelApps (ModelID, App) VALUES ((SELECT ModelID FROM Models WHERE Name = '" & $sModelName & "'),'" & $sModelApp & "')"
                _SQLite_Exec($hDb, $sQuery)
                If @error Then
                ...
                $sQuery = "INSERT INTO ModelFocuses (ModelID, Focus, Stems) VALUES ((SELECT ModelID FROM Models WHERE Name = '" & $sModelName & "'),'" & $sModelFocus & "'," & $iModelStems & ")"
                _SQLite_Exec($hDb, $sQuery)
                If @error Then
...
Func _TabHandler()
    ...
    If $hModelCombo <> 0 Then
        ...
        If $hSegmentSizeLabel <> 0 Then GUICtrlDelete($hSegmentSizeLabel)
        If $hSegmentSizeInput <> 0 Then GUICtrlDelete($hSegmentSizeInput)
        If $hOverlapLabel <> 0 Then GUICtrlDelete($hOverlapLabel)
        If $hOverlapInput <> 0 Then GUICtrlDelete($hOverlapInput)
        If $hDenoiseCheckbox <> 0 Then GUICtrlDelete($hDenoiseCheckbox)
        If $hBatchSizeLabel <> 0 Then GUICtrlDelete($hBatchSizeLabel)
        If $hBatchSizeInput <> 0 Then GUICtrlDelete($hBatchSizeInput)
        If $hAggressivenessLabel <> 0 Then GUICtrlDelete($hAggressivenessLabel)
        If $hAggressivenessInput <> 0 Then GUICtrlDelete($hAggressivenessInput)
        If $hTTACheckbox <> 0 Then GUICtrlDelete($hTTACheckbox)
        If $hHighEndProcessLabel <> 0 Then GUICtrlDelete($hHighEndProcessLabel)
        If $hHighEndProcessCombo <> 0 Then GUICtrlDelete($hHighEndProcessCombo)
        $hSegmentSizeLabel = 0
        $hSegmentSizeInput = 0
        $hOverlapLabel = 0
        $hOverlapInput = 0
...
Func _MDXSettingsHandler()
    ...
    If Not StringIsInt($sSegmentSize) Or $sSegmentSize <= 0 Then
        ...
        GUICtrlSetData($hSegmentSizeInput, IniRead($sSettingsIni, "MDXNet", "SegmentSize", "256"))
        Return
    EndIf

    If Not StringIsFloat($sOverlap) Or $sOverlap < 0 Or $sOverlap > 1 Then
        _Log("Invalid Overlap: " & $sOverlap, True)
        MsgBox($MB_ICONWARNING, "Warning", "Overlap must be a number between 0 and 1.")
        GUICtrlSetData($hOverlapInput, IniRead($sSettingsIni, "MDXNet", "Overlap", "0.25"))
        Return
    EndIf
    ...
    If Not StringIsInt($sBatchSize) Or $sBatchSize <= 0 Then
        ...
        GUICtrlSetData($hBatchSizeInput, IniRead($sSettingsIni, "MDXNet", "BatchSize", "1"))
        Return
    EndIf

    If Not StringIsInt($sAggressiveness) Or $sAggressiveness < 0 Then
        _Log("Invalid Aggressiveness: " & $sAggressiveness, True)
        MsgBox($MB_ICONWARNING, "Warning", "Aggressiveness must be a non-negative integer.")
        GUICtrlSetData($hAggressivenessInput, IniRead($sSettingsIni, "MDXNet", "Aggressiveness", "10"))
        Return
    EndIf
    ...
    _Log("Saved MDX settings: SegmentSize=" & $sSegmentSize & ", Overlap=" & $sOverlap & ", Denoise=" & $bDenoise & ", BatchSize=" & $sBatchSize & ", Aggressiveness=" & $sAggressiveness & ", TTA=" & $bTTA & ", HighEndProcess=" & $sHighEndProcess)
...
Func _DescEditHandler()
    ...
    EndIf
    Local $sDescription = GUICtrlRead($hDescEdit)
    _Log("Description updated for model " & $sModel & ": " & $sDescription)
    _SaveModelDetails($sModel, $sDescription, GUICtrlRead($hCommentsEdit))
...
Func _CommentsEditHandler()
    ...
    EndIf
    Local $sComments = GUICtrlRead($hCommentsEdit)
    _Log("Comments updated for model " & $sModel & ": " & $sComments)
    _SaveModelDetails($sModel, GUICtrlRead($hDescEdit), $sComments)
